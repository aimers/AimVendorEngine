...
sap.ui.medApp.global.globalFormatter = {
	...
	getDistance : function(oArray) {
		if (oArray != null && oArray != undefined) {
			...
			var dist = sap.ui.medApp.global.util.distance(latt, longt, longt1,
					latt1, "K");
	...
	getBookingHistory : function(value) {
		var arr = [];
		if (value) {
			var oData = this.oModel.getProperty("/bookingHistory");
			If(oData instanceof Array)
			{
				oData.foreach(function(oValue, i) {
					if (oValue.BOSTM === value) {
						arr.push(oValue);
					}
				});
